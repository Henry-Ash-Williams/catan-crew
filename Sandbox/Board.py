...
class Board:
  ...
  def select(board, around, distance, dir_pattern = None, matching = lambda o: True, return_cells=False):
    ...
    else: directions = [sum(board.cardinal_directions[(i+j)%6]*dir_pattern[j] for j in range(len(dir_pattern)))%board.cell_count for i in range(6)]
    
    out = [(around + distance*directions[i] + directions[(i+2)%6]*j)%board.cell_count for i in range(6) for j in range(distance)]
  ...
  def add_road(board, location, road):
    ...
    neighboring_intersections = board.select(location, 1, matching = board.has_intersection, return_cells = True)
