...
class Board:
    ...
    def __init__(board, size=3, initial_data=None):
        ...
        board.tiles = join(board.select(0, i, dir_pattern=(2, 2)) for i in range(n)) + [
            0
        ]
        ...
        resources = (
            ["grain"] * 4 + ["wool"] * 4 + ["lumber"] * 4 + ["brick"] * 3 + ["ore"] * 3
        )
        ...
        for location in board.tiles:
            resource = (
                "nothing" if (location in board.desert_tiles) else board.resources.pop()
            )
            number_token = (
