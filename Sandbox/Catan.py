...
class Board:
  ...
  def is_path(self, cell_num): return type(self.cells[cell_num]).__name__ == 'Path'
  
  def is_intersection(self, cell_num): return type(self.cells[cell_num]).__name__ == 'Intersection'
...
for cell_num in range(newBoard.mod):
  if newBoard.is_intersection(cell_num):
    if newBoard.cells[cell_num].has_settlement:
      print(cell_num, newBoard.cells[cell_num].settlement.color, type(newBoard.cells[cell_num].settlement).__name__)